...
- **2026-09-02 — Dirección propuesta (pendiente de confirmación del usuario) para resolver de raíz la ventana de favoritos en Liga/Copa Betplay: mover fixtures/hora/resultado a una API estructurada (`api-football.com` / `api-sports.io`), dejando la IA solo para resolver el canal.** Investigado: `api-football.com` (== `api-sports.io`) ofrece un plan gratuito real (100 solicitudes/día, sin tarjeta para registrarse) y su documentación indica cobertura de Colombia Primera A (Liga Betplay). No se confirmó todavía si el plan gratuito incluye Copa Colombia (Copa Betplay) ni si hay restricciones de temporada para ligas no-top en el tier gratuito — la página de coverage bloqueó el fetch automatizado (403) y las búsquedas web no fueron concluyentes en ese detalle. **Por qué esto es mejor que seguir afinando la extracción por IA:** separaría el dato de alto riesgo (fecha/hora/marcador — un error ahí ya causó el incidente que originó todo este cambio) del de bajo riesgo (canal — si falla, el partido simplemente queda con `canal: null`, que la app ya maneja bien). Es exactamente el mismo patrón que ya funciona para los 4 torneos europeos (sección 5) aplicado a Liga/Copa Betplay. **Siguiente paso concreto:** el usuario debe crear la cuenta gratuita en api-football.com (sin tarjeta, sección 3 restricción #2) y pasar la API key para probar en vivo qué trae realmente el tier gratuito para Colombia antes de construir el pipeline sobre esto.
