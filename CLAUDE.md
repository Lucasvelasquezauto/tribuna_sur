...
- **2026-09-02 — El usuario notó que de los 4 partidos de HOY (Liga+Copa Betplay) en la vista "Todos", 2 eran falsos. Investigación con dos capas de causa raíz distintas, corregidas ambas en el prompt (el usuario rechazó explícitamente arreglarlo con un override: "no lo arregles con parche... lo que necesitamos es entender por qué pasó y cómo evitamos que vuelva a pasar").**
  ...
  - Estado final verificado con 3 corridas seguidas del pipeline completo tras los dos arreglos: los 2 falsos confirmados (Pasto-Medellín, Fortaleza-Once Caldas) ya no se reproducen; la salvaguarda de duplicados siguió atajando a Nacional-Cali (que la búsqueda de copa-betplay sigue proponiendo ocasionalmente para hoy, en vez de su fecha real del 7-sep) en las 3 corridas, consistente con el diseño de "salvaguarda como respaldo, no como mecanismo principal".
- **2026-09-02 — CORRECCIÓN de la entrada anterior: "Cúcuta vs Internacional de Bogotá" NO era real para hoy — me equivoqué DOS VECES seguidas sobre el mismo partido, y el usuario lo notó antes que yo ("No. Hoy no juega ni jugó el cúcuta").** Reconstruyendo la cadena de error: (1) primera verificación, correcta (365scores: 19 de agosto) → lo borré. (2) Al investigar más, encontré un artículo diciendo "se reprogramó para el 2 de septiembre por las Ferias de Cúcuta" + una crónica de WinSports en pasado ("Cúcuta rescató el empate") y concluí que SÍ era de hoy → lo reinserté. **Estaba mal**: una tercera fuente (Caracol Radio, con la fecha **en la propia URL**, `caracol.com.co/2026/08/19/...`) tenía el mismo relato exacto (mismo marcador, mismo gol de penalti de Dájome) fechado el 19 de agosto — la crónica de WinSports que tomé como "de hoy" en realidad describía ESE partido de agosto, no uno de septiembre. El anuncio de "reprogramado para el 2 de septiembre" fue un plan que después quedó superado por la fecha real (19 de agosto). Confirmé además revisando la hora real de Bogotá con `Intl.DateTimeFormat` (Node, confiable) en vez de `TZ=America/Bogota date` del shell de Git Bash en Windows, que **no tiene tzdata y devuelve UTC silenciosamente mal etiquetado como hora local** — otra fuente de confusión propia en el camino, ya corregida (usar Node para cualquier cálculo de hora de Bogotá, nunca el `date` del shell en este entorno).
  - **Se borró definitivamente** (partido id 138). Se agregó una regla nueva en `gemini.js`: un anuncio de reprogramación futura ("se reprogramó para X") es un PLAN, no un hecho confirmado, y una crónica YA PUBLICADA de un partido (con su propia fecha, ej. en la URL) manda sobre cualquier plan de reprogramación a otra fecha, sin importar cuál de los dos artículos suene más reciente o específico.
  - **El usuario, con razón, no se conformó con "ya lo arreglé" otra vez** ("Esta app no me está generando confianza... Se supone que solo con entrar a la app voy a saber quien juega") y señaló el problema de fondo: agregar más fuentes (Tavily trayendo 6-8 resultados de todo internet por búsqueda) obliga al modelo a reconciliar señales de calidad muy dispar, y ahí es donde aparecieron los bugs de la noche. Pidió explícitamente **"escoger fuentes confiables, no múltiples"**.
- **2026-09-02 — Cambio de mecanismo (no solo de prompt): `buscarTextoTavily` ahora restringe la búsqueda a una lista curada de dominios confiables (`include_domains`) en vez de todo internet, más filtros de recencia/tema/país.** Acordado con el usuario: para "hoy" se evaluó volver a una fuente única (`colombia.com`, nunca dio un dato incorrecto en toda la sesión) en vez de Tavily, pero el usuario prefirió mantener Tavily para poder seguir cubriendo la ventana de +14 días, a condición de mejorar sus parámetros de búsqueda en vez de solo parchar el prompt cada vez.
  - `DOMINIOS_CONFIABLES` en `scripts/lib/tavily.js`: dimayor.com.co, winsports.co, futbolred.com, colombia.com, noticiascaracol.com, caracol.com.co, elespectador.com, elpais.com.co, eltiempo.com, semana.com, as.com, espn.com.co (nota: `espn.com.co` es distinto de `espndeportes.espn.com`, que SÍ se excluyó). Deliberadamente **excluidos** por causar bugs reales esta sesión: `espndeportes.espn.com`/`espn.com` (tablas-calendario de un equipo con decenas de fechas, causa raíz de 2 de los 3 bugs de hoy), `sofascore.com` (horas en UTC sin dejarlo claro, causó el bug de Fortaleza-Once Caldas), `valuestats.com` (horarios inconsistentes).
  - Parámetros nuevos en la llamada a Tavily: `topic: 'news'` (prioriza articulos de noticias sobre paginas de referencia/calendario estatico), `country: 'colombia'`, `time_range: 'month'` (se probó `'week'` primero, pero redujo demasiado la cobertura de fechas lejanas -- ver nota abajo).
  - **Verificado con el pipeline completo**: "hoy" (2-sep) volvió a dar exactamente los 2 partidos reales, sin ninguno de los 3 falsos de esta noche, usando ahora un PDF oficial de confirmación de Dimayor y un articulo de Noticias Caracol fechado "HOY" como fuentes -- mucho mas limpio que antes.
  - **Trade-off real encontrado y aceptado, no oculto:** con dominios+recencia acotados, Tavily encuentra MENOS para fechas lejanas (+7 a +13 dias) -- en las pruebas, cayo a 0 resultados para el 7-sep y 13-sep, y el pipeline recurrio al candidato de respaldo (el hub de Dimayor, que solo funciona local/manualmente, no en CI). Investigando por que: la informacion SI existe (ej. un articulo de El Tiempo "Dimayor confirma la programacion oficial de la fecha 9"), pero la tabla de partidos esta embebida como IMAGEN en el articulo ("Programacion fecha 9. Foto:Dimayor"), no como texto -- Tavily no puede leer una imagen. No es un bug de nuestro pipeline, es un limite real de que informacion existe en texto extraible. Consistente con "mejor omitir que adivinar": en produccion, es esperable que la ventana de +14 dias tenga mas huecos (partidos que no aparecen hasta mas cerca de la fecha) que antes de este cambio, a cambio de que lo que SI aparece sea mas confiable. El presupuesto de creditos de Tavily no cambia (~774/mes, cada intento cuesta lo mismo aunque no encuentre nada).
- **2026-09-02 — Relleno histórico de los últimos 14 días (Liga/Copa Betplay) para arreglar la ventana de favoritos de equipos poco probados esa noche (ej. Independiente Medellín solo tenía 1 partido guardado en toda la BD).** Causa raíz: el cron nuevo (Tavily) solo busca hacia ADELANTE, nunca hacia atrás, y empezó a correr recién esa noche -- el "pasado" de la ventana de favoritos dependía de que fechas se hubieran probado a mano, no de un mecanismo real. Se corrió `fetch-ia` para los 11 días sin cobertura (19-25, 27, 28, 30, 31 de agosto) vía el mecanismo normal (no a mano), poblando 33 partidos reales. **No hace falta que esto se repita**: de ahora en adelante cada "hoy" que el cron captura se vuelve "pasado" solo con el paso de los dias, la ventana se autocompleta.
  - De paso se encontró y fusionó otro duplicado del mismo patrón que Llaneros/Santa Fe: "Bogotá" vs "Internacional de Bogotá" (equipo id 182 fusionado hacia 177).
  - Quedó un residuo aceptado: 9 partidos de hace 10-14 dias siguen `programado` sin marcador final (el mecanismo de cierre normal solo mira los ultimos 3 dias, nunca los tocó) -- dato incompleto, no incorrecto, no urgente.
- **2026-09-02 — El usuario notó que Independiente Medellín (recien favoriteado) solo mostraba 1 partido dentro de la ventana +14, pidió investigar sin parchar a mano.** Causa real, no adivinada: revisando `fetch_jobs`, 2 fechas puntuales (liga-betplay 9-sep y 12-sep) habían quedado en estado `error` durante la corrida de esa misma tarde -- coincide exactamente con el momento en que se estaban haciendo decenas de pruebas manuales seguidas contra Gemini, que respondio 429/503 (limite de cuota) en esos intentos especificos. No es un bug del pipeline: fue saturacion transitoria de mi propio uso intensivo esa noche, en fechas que nunca se reintentaron despues. Confirmado reintentando esas 2 fechas bajo condiciones normales: encontraron los partidos reales del DIM (12-sep vs Boyacá Chicó; 9-sep vs Pasto en Copa Betplay, vuelta de octavos) sin problema. **No se necesita ningun cambio de codigo para esto** -- el cron de produccion vuelve a barrer los 14 dias completos 3x/semana, asi que una falla transitoria en una fecha puntual se reintenta sola en la siguiente corrida programada, antes de que llegue el dia del partido. El usuario pidio ademas reintentar las 8 fechas restantes que habian quedado en error de la corrida de esa tarde -- se corrieron todas (algunas se resolvieron, otras genuinamente no tenian partido esos dias, un par quedó con timeout del hub de Dimayor en ese momento puntual, sin relacion con el codigo).
- **2026-09-02 — Idea del usuario: para Copa Betplay en fase eliminatoria (octavos), en vez de 14 busquedas ancladas a una fecha (que fallan seguido por como esta fragmentada la cobertura de este torneo especifico, ver seccion 10), agregar una busqueda del CUADRO COMPLETO de la fase.** Como Copa Betplay en octavos tiene un numero fijo y conocido de cruces (8, con ida y vuelta cada uno), una sola busqueda sin fecha especifica ("Copa Betplay Colombia 2026 octavos de final programacion completa todos los partidos fecha hora canal") trae los 8 cruces completos desde 1-2 fuentes (confirmado: un articulo de ESPN y una confirmacion oficial de Dimayor via El Tiempo, ambos con los 8 cruces, fechas de ida/vuelta y estadios).
  - `scripts/lib/tavily.js`: nueva funcion `queryCuadroCopaBetplay()`. **Mantenimiento pendiente:** el texto de la query tiene "octavos de final" hardcodeado -- hay que actualizarlo a mano cuando el torneo avance a cuartos/semifinal/final, no hay forma automatica de saber en que fase esta.
  - `scripts/fetch-ia.js`: se agrega como candidato de respaldo SOLO para copa-betplay, despues de la busqueda por fecha especifica y antes del hub de Dimayor -- no se corre en cada fecha (para no duplicar el gasto de creditos en los dias que la busqueda por fecha ya resuelve bien), solo cuando esa busqueda no encuentra nada.
  - **Verificacion parcial:** confirmado que funciona para el 3 de septiembre (encontro los 2 partidos correctos via el cuadro completo, y la salvaguarda de duplicados correctamente evito guardarlos en esta fecha porque ya existian con su fecha real en otro lado). El resto de las pruebas de esa noche se contaminaron con limite de cuota de Gemini (15 solicitudes/minuto en el tier gratis, agotado por el volumen de pruebas manuales de toda la sesion) -- no se pudo verificar limpio mas alla de ese caso, pero el mecanismo y el codigo son solidos; la proxima corrida real del cron (sin la saturacion de esta noche) es la verificacion pendiente.
