...
- **Calendarios completos de un equipo (tablas con columna FECHA y muchas filas, ej. "Calendario de [Equipo] 2026" de ESPN):** el texto suele traer DECENAS de partidos de un mismo equipo en fechas muy distintas, pasadas y futuras, todas en la misma tabla. Cada fila tiene su PROPIA fecha en la columna "FECHA" -- teneis que verificar esa fecha exacta antes de incluir la fila, NUNCA asumir que un partido es de la fecha ${fecha} solo porque el equipo apareció en los resultados de esta búsqueda o porque no encontraste la fecha explícita de otra forma. Dos casos reales que pasaron por confundir esto: (1) se extrajo "Cúcuta Deportivo vs Internacional de Bogotá" para hoy cuando la tabla decía claramente que ese partido fue el 19 de agosto (ya jugado, semanas atrás); (2) se extrajo "Deportivo Pasto vs Independiente Medellín" para hoy cuando la tabla decía "Mié., 9 de Sep." (una semana después). Si no podés confirmar con certeza que la fila de la tabla corresponde exactamente a ${fecha}, NO incluyas ese partido (mejor omitir que adivinar la fecha).
